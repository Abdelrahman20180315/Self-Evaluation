...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  <head>
        ...
        <title>  إضافة وعرض الاسئلة</title>
        <!--     Fonts and icons     -->
          ...
          @include('Admins.adminnav')
            <div class="card" style="width: 80%;margin-left:10%;">
              ...
              <div style="text-align:right;" class="card-header">
                 <a href="{{url('/creategroups/selectedprgram',$group->program->id)}}" class="btn btn-success">عودة الي الصفحة السابقة</a>
                 <h4 class="text-xs font-weight-bold" align:center> مجموعة  {{$group->group_name}} </h4>
                 <br>
                 <h4> إضافة وعرض الاسئلة</h4>
              </div>
      
                @livewire('makequestions',['group_id' => $group_id])
